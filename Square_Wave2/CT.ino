float S = 0, E = 36.525, threshold, previous_current = 0;
int analogValue;
void A7_init() {
...
void currentMeasure() {
  ...
  CT_time = micros();
  threshold = 0.2 * previous_current;  // 20% of previous current
  current = 0;
  for (int i = 0; i < 10; i++) {
  ...
  if (current >= (previous_current - threshold) && current <= (previous_current + threshold)) {
    current = (current + previous_current) / 2.0;  // Take the average
  } else {
    current = current;  // Use the new value directly
  }

  previous_current = current;  // Update previous current
...
//    Serial.print("  T: ");
//    Serial.println(t2-CT_time);
//  
//    if(current>=1.5) digitalWrite(13,HIGH);
//    delay(500);
}
